					...
					<div class="jumbotron" style="padding:25px;">
						<h4 class="pull-left" style="padding-right: 15px;">
							<?php 
								$u_id = $_GET['id'];
								$ssid = 0;
								$select_p = $conn->query("SELECT * FROM i WHERE id=$u_id");
						...
						<table class="table">
							...
							<?php 
								...
								$select_legal = $conn->query("SELECT * FROM legal_law WHERE user_id=$u_id ORDER BY id DESC");
							...
							<tr>
								...
								<td>
									<?php 
										if($row_legal->author_id==$logged_id){
									?>
									<a href="delete_p.php?id=<?php echo $row_legal->id; ?>" class="btn btn-danger">
										წაშლა
									</a>
									<a href="edit_p.php?id=<?php echo $row_legal->id; ?>" class="btn btn-warning">
										რედაქტირება
									</a>
									<?php } ?>
								</td>
