...
<center><p style="color: red;">(ტექნიკური დახმარებისთვის, დააჭირეთ ღილაკს, ან პრობლემა მოგვწერეთ მეილზე: tedo.tandilashvili@gmail.com)</p></center>
    ...
    <form name="search" action="" method="POST">
        <input type="date" name="date_a" class="form-control" value="<?php echo date("Y-m-d"); ?>">
        <br />
        <input type="date" name="date_b" class="form-control" value="<?php echo date("Y-m-d"); ?>">
        <br />
        <input type="submit" name="submit" class="form-control" value="ძებნა">
    </form>
    <?php
        if(isset($_POST['submit']) && $_POST['submit']=="ძებნა"){
            $date_a = $_POST['date_a'];
            $date_b = $_POST['date_b'];
  ...
  <table class="table" id="myTable">
    ...
    <tbody>
        ...
        $c1 = 0;
        $c2 = 0;
        $select = $conn->query("SELECT * FROM chanaweri WHERE gadamcemi!='' AND cspdate>='$date_a' AND cspdate<='$date_b' ORDER BY cspdate DESC ");

        
        while($arr = $select->fetch_object()){
            if($arr->cspstatus==1){
                $c1++;
            }else{
                $c2++;
  ...
  <?php 
  echo "ჩართული: " . $c1 . "<br />" . "გამორთული: " . $c2;
  ?>
