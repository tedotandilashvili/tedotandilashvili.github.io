...
{	
	...
	if(empty($name) || empty($price) || empty($saleprice)) {	
		...
		//updating the table
		$result = mysqli_query($mysqli, "UPDATE ekacamip SET name='$name',price='$price',saleprice='$saleprice', cat = '$cat' WHERE id=$id");
...
$result = mysqli_query($mysqli, "SELECT * FROM ekacamip WHERE id=$id");
